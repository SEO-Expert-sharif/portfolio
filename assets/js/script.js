(function ($) {
    ...
    $(document).ready(function () {
        ...
        emailjs.init("HFmcneyq5QL_tdXEp"); // EmailJS public key

        $('#contact-form').on('submit', function (e) {
            e.preventDefault();

            let userName = $('#fname').val();
            let userEmail = $('#umail').val();
            let userPhone = $('#phone').val();
            let userMessage = $('#message').val();

            // 1️⃣ Send data to your inbox (rahat@gmail.com)
            let adminParams = {
                from_name: userName,
                from_email: userEmail,
                phone: userPhone,
                message: userMessage,
                to_email: "reset.rahat@gmail.com" // তোমার inbox
            };

            emailjs.send('service_pn726u8', 'template_u73cwcd', adminParams)
            ...
            emailjs.send('service_pn726u8', 'template_taa54k9', userParams)
                .then(function (response) {
                    alert('Thank you! Your message has been sent and a confirmation email is sent to you.');
                }, function (error) {
